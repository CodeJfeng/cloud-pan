...
public class UserController {
     ...
     */
    @LoginIgnore
    @RateLimit(key = "user_register", permits = 5, seconds = 300, limitType = RateLimit.LimitType.IP, message = "注册请求过于频繁，请稍后重试")
    @Operation(summary = "用户注册接口", description = "提供用户登录接口，实现了幂等注册的逻辑，可以放心调用")
     ...
     */
    @LoginIgnore
    @RateLimit(key = "user_login", permits = 10, seconds = 300, limitType = RateLimit.LimitType.IP, message = "登录请求过于频繁，请稍后重试")
    @Operation(summary = "用户登录接口", description = "该接口提供了用户登录功能，成功登录以后，会返回具有时效性的accessToken", responses = {
     ...
     * @return 返回退出成功信息
     */
    @Operation(summary = "用户登出接口", description = "该接口提供了用户登出账号的功能")
    @PostMapping("exit")
    public R exit() {
     ...
     */
    @LoginIgnore
    @Operation(summary = "用户校验密码--校验用户名称", description = "该接口提供了用户在忘记密码时，验证用户名是否存在")
     ...
     */
    @LoginIgnore
    @Operation(summary = "用户忘记密码--校验密保答案", description = "该接口提供了用户在忘记密码时，验证校验密保答案是否正确")
     ...
     */
    @LoginIgnore
    @Operation(summary = "用户忘记密码--更新密码", description = "该接口提供了用户在忘记密码时，通过token去更新密码信息")
     ...
     * @return
     */
    @Operation(summary = "用户在线更新密码", description = "该接口提供了用户在线更新密码")
    @PostMapping("password/change")
    public R changePassword(@Validated @RequestBody ChangePasswordPO changePasswordPO) {
        ChangePasswordContext changePasswordContext = userConverter
                .changePasswordPO2ChangePasswordContext(changePasswordPO);
     ...
     * @return
     */
    @Operation(summary = "查询用户的基本信息", description = "该接口提供了查询用户的基本信息")
    @GetMapping("/")
