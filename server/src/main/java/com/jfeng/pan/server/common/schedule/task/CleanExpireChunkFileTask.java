...
public class CleanExpireChunkFileTask implements ScheduleTask {
     ...
     * 执行清理任务
     * 1、滚动查询过期的文件分片
     * 2、删除物理文件——委托物理文件引擎去实现
     * 3、删除过期文件分片的记录信息
     * 4、重置上次查询的最大文件分片记录ID、继续滚动查询——批量滚动查询
     */
    @Override
    public void run() {
        ...
        do{
            ...
            if (CollectionUtil.isNotEmpty(expireFileChunkRecords)){
        ...
        }while(CollectionUtil.isNotEmpty(expireFileChunkRecords));
...
/**************************************  private   **********************************************************************/
    ...
    private void deleteRealChunkFiles(List<RPanFileChunk> expireFileChunkRecords) {
        ...
        List<String> realPathList = expireFileChunkRecords.stream().map(RPanFileChunk::getRealPath).toList();
